...
- **Intro-Playthrough** (`introPlaythrough: true`, NUR team.html): beim Öffnen spielt das Reel EINMAL komplett durch (`video.play()` 0→Ende), erst danach übernimmt der Scroll-Scrub. Ablauf: `canplaythrough` → `startIntro()` (play), `introActive` lässt `tick()` früh returnen (Scrub pausiert). **Übergabe an den Scrub NUR bei `ended`** (bewusst KEIN Scroll-Hand-off — Scrollen während des ersten Durchlaufs lässt das Reel ungestört zu Ende laufen, die Seite scrollt trotzdem). `endIntro()`: pausiert, setzt `smoothedProg = currentTime/duration` und öffnet ein ~1.4s **postIntro-Settle**-Fenster (hält den Scrub aktiv, damit das Video vom Endframe sanft auf die aktuelle Scroll-Position lerpt statt hart zu springen). Not-Start nach 6s (falls `canplaythrough` nie feuert) + 32s-Backstop für `ended`. **Reduced-motion**: kein Auto-Intro (`introActive = introPlaythrough && !reduce`), direkt Scrub. **Extraktion ist im Intro-Modus AUS** — der native keyint=1-Seek (team-reel ist all-I-frame) scrubbt smooth genug, das spart den Freeze-Frame-Konflikt mit dem Playthrough.
- **Nicht-Scrub-Hintergründe** (`.bg-loop` auf about/partner/kontakt): einfache, fixe Endlos-Videos OHNE Scroll-Kopplung. `<video autoplay loop muted playsinline>` + `.bg-scroll-tint` für Lesbarkeit; main.js stupst `play()` an. **Ping-Pong (vor→zurück→vor…)**: KEIN negativer `playbackRate` (browserweit nicht unterstützt) und KEIN manuelles Rückwärts-Seeken (ruckelt) — stattdessen eine **vorgerenderte Boomerang-Datei** (ffmpeg `split` → `reverse` → `concat=n=2`) nativ loopen. Smooth, iOS-sicher, kein JS. Befehl: `ffmpeg -i src -filter_complex "[0:v]scale=1920:-2,fps=24,split[a][b];[b]reverse[r];[a][r]concat=n=2:v=1[v]" -map "[v]" -an -c:v libx264 -crf 23 -movflags +faststart out.mp4`. **Mobile-Bild-Ausschnitt** (z. B. kontakt): bei 16:9-Clip im Portrait-Cover wird nur der Mittel-Slice gezeigt — mit `object-position` (Modifier-Klasse `.bg-loop__video--front`, nur im ≤900px-Media-Query) das wichtige Motiv (z. B. die Auto-Front links) ins Bild holen.
- **Reel-reactive Light** (team.html): das Video TREIBT die Seite, synchron zur Schwarz→Weiß-Wagen-Stelle im Clip (~10.6–13.2s). main.js liest in einer rAF-Loop `#teamBgVideo.currentTime` und setzt `--reel-flash` (0..1) **kontinuierlich** via Smoothstep-Hüllkurve (steigt wenn Wagen weiß wird, fällt wenn wieder dunkel). CSS macht daraus EINEN **dünnen Streifen** (`.reel-strip`) der von oben nach unten wandert und die Seite dahinter in einen helleren Ton **kleidet** (`.reel-wash`), dann wieder umgekehrt. **Pflicht**: `mix-blend-mode: screen` für Streifen UND Wash — `screen` ADDIERT nur Licht, verdeckt das Video also NIE (User-Anforderung: Video muss sichtbar bleiben). KEINE Text-Invertierung mehr (würde das Video verdecken). Läuft für Scrub (Desktop) UND Loop (Mobile). Hüllkurven-Zeiten an die Video-Inhalte anpassen.
...
- **Section-Containment (Pflicht-Details)**: Sizing auf `canvas.clientWidth/Height` (nicht `window`). **Render-Gating** per `IntersectionObserver` — WebGL pausiert, wenn Section off-screen (`if (!sectionVisible) return;` in der rAF-Loop). **Picking section-relativ**: Pointer-Events auf `window`, `closest(INTERACTIVE_SEL)` schließt UI aus, Treffer muss INNERHALB `canvasRect` liegen, NDC aus `canvasRect`-Offset. `canvasRect` per Frame (sichtbar) + on scroll/resize aufgefrischt (Section scrollt mit).
- **Tap vs. Drag (Maus + Touch, einheitlich)**: Picking läuft über `pointerdown`+`pointerup` (NICHT `click` — auf Touch unzuverlässig, Mobile-Planeten waren sonst nicht wählbar). `pointerup` mit Bewegung < 14px UND < 700ms = **Tap → Planet wählen**; mehr Bewegung = **Drag → Orbit** (System rotiert: `userRotY/userRotX` akkumulieren in `pointermove`, im Render gelerpt auf `system.rotation`). Canvas dafür `pointer-events: auto; touch-action: pan-y` (horizontaler Touch-Drag orbitet, vertikaler scrollt die Seite weiter; Desktop-Maus orbitet in beiden Achsen).
